import os
# Set MuJoCo to use EGL for headless rendering (must be before mujoco import)
    ...
    while episode_count < MAX_EPISODES:
        ...
        while True:
            ...
            if step_count % 67 == 0: 
                ...
                # Get full 6D pose: position (3D) + euler angles (3D)
                ee_pos = data.site_xpos[tip_id].copy()  # XYZ position
                ee_mat = data.site_xmat[tip_id].reshape(3, 3)  # Rotation matrix

                # Convert rotation matrix to Euler angles (XYZ convention)
                ee_euler = np.zeros(3)
                mujoco.mju_mat2Quat(np.zeros(4), ee_mat.flatten())  # First convert to quat
                # Better: use scipy or direct conversion
                # For now, we can use MuJoCo's built-in conversion

                # Alternative: Store quaternion and convert later
                # For consistency with real robot (which uses euler), let's convert
                ee_quat = np.zeros(4)
                mujoco.mju_mat2Quat(ee_quat, ee_mat.flatten())

                # Convert quaternion to Euler angles (ZYX order, commonly used in robotics)
                # This matches the real robot's RPY output
                r = np.sqrt(1 + ee_mat[0,0] + ee_mat[1,1] + ee_mat[2,2]) / 2
                i = (ee_mat[2,1] - ee_mat[1,2]) / (4*r)
                j = (ee_mat[0,2] - ee_mat[2,0]) / (4*r)
                k = (ee_mat[1,0] - ee_mat[0,1]) / (4*r)
                quat_norm = np.array([i, j, k, r])

                # Simple euler extraction from matrix (more reliable)
                sy = np.sqrt(ee_mat[0,0]**2 + ee_mat[1,0]**2)
                singular = sy < 1e-6

                if not singular:
                    roll = np.arctan2(ee_mat[2,1], ee_mat[2,2])
                    pitch = np.arctan2(-ee_mat[2,0], sy)
                    yaw = np.arctan2(ee_mat[1,0], ee_mat[0,0])
                else:
                    roll = np.arctan2(-ee_mat[1,2], ee_mat[1,1])
                    pitch = np.arctan2(-ee_mat[2,0], sy)
                    yaw = 0

                ee_euler = np.array([roll, pitch, yaw])  # radians
                ee_pose_6d = np.concatenate([ee_pos, ee_euler])  # (6,) array: [x, y, z, rx, ry, rz]
