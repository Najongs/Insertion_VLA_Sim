...
class HDF5LeRobotDataset(Dataset):
    ...
    def __init__(
        ...
        # Read language instruction from HDF5 if available (for variable instructions per episode)
        # If not present, generate instruction from phase information
        if 'language_instruction' in self.h5file:
            try:
                instruction_bytes = self.h5file['language_instruction'][()]
                if isinstance(instruction_bytes, bytes):
                    self.task = instruction_bytes.decode('utf-8')
                else:
                    self.task = str(instruction_bytes)
                logger.info(f"Episode {episode_index}: Using instruction from HDF5: {self.task}")
            except Exception as e:
                logger.warning(f"Episode {episode_index}: Failed to read instruction from HDF5: {e}. Using phase-based instruction.")
                self.task = self._generate_instruction_from_phase()
        elif 'phase' in self.h5file:
            # Generate instruction from phase information (3-state task)
            self.task = self._generate_instruction_from_phase()
            logger.info(f"Episode {episode_index}: Using phase-based instruction: {self.task}")
        else:
            # Fallback to provided task_instruction
            self.task = self.default_task_instruction
            logger.info(f"Episode {episode_index}: Using default instruction: {self.task}")
