...
def run_validation(model, val_dataloader, accelerator):
    ...
    avg_val_loss = total_val_loss / num_val_batches if num_val_batches > 0 else torch.tensor(0.0, device=accelerator.device)

    # Use reduce instead of gather to avoid timeout issues
    # This is more efficient and less prone to hanging
    avg_val_loss = accelerator.reduce(avg_val_loss, reduction="mean")
    ...
    while completed_steps < config.max_train_steps:
        for batch in train_dataloader:
            with accelerator.accumulate(model):
                ...
                outputs = model(**batch)
                loss = outputs.loss
                total_loss += loss.detach().float().item()  # FIXED: Convert to Python float immediately
                ...
                if accelerator.sync_gradients:
                    ...
                    if completed_steps % config.logging_frequency == 0:
                        if accelerator.is_main_process:
                            ...
                            logger.info(f"Step {completed_steps}, Loss: {loss.item():.4f}, Grad Norm: {total_norm:.4f}, LR: {lr:.6f}")
                ...
                # FIXED: Periodic garbage collection to prevent memory accumulation
